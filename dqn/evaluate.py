...
parser.add_argument(
    ...
    dest="save_video",
    action="store_true",
    help="Save the episodes as video.",
)
parser.set_defaults(render=False)
parser.set_defaults(save_video=False)

# Hyperparameter configurations for different environments. See config.py.
ENV_CONFIGS = {"CartPole-v0": config.CartPole, "CartPole-v1": config.CartPole}


def evaluate_policy(
    dqn, env, env_config, args, n_episodes, render=False, verbose=False
):
...
if __name__ == "__main__":
    ...
    mean_return = evaluate_policy(
        ...
        render=True and not args.save_video,
        verbose=True,
    )
    print(
        f"The policy got a mean return of {mean_return} over {args.n_eval_episodes} episodes."
    )
